...
function teamstest() {
    ...
    //all possible teams
    var allPossibleTeams = new Iter(players).combinations(5).toArray();
    ...
    var picker
    if (teamP == 0)
        picker = blueTeam[Math.floor(Math.random() * 5)].username
    else 
        picker = redTeam[Math.floor(Math.random() * 5)].username
    const teams = {
        redTeam: rT,
